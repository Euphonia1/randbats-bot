...
def build_attacker(state, side) -> Attacker:
    i = act(state, side)
    return Attacker(
        level=state.level[side, i], stats=state.stats[side, i],
        boosts=state.boosts[side], types=active_types(state, side),
        base_types=state.types[side, i], ability=state.ability[side, i],
        item=state.item[side, i], status=state.status[side, i],
        hp=state.hp[side, i], maxhp=state.maxhp[side, i],
        terastallized=state.terastallized[side, i], tera_type=state.tera_type[side, i],
    )
...
def build_defender(data, state, side) -> Defender:
    i = act(state, side)
    return Defender(
        stats=state.stats[side, i], status=state.status[side, i],
        boosts=state.boosts[side], types=active_types(state, side),
        ability=state.ability[side, i], item=state.item[side, i],
        hp=state.hp[side, i], maxhp=state.maxhp[side, i],
        terastallized=state.terastallized[side, i], tera_type=state.tera_type[side, i],
        nfe=data["species_nfe"][state.species[side, i]],
    )


def build_cb_ctx(data, state, user, target, moves_first, pp_left, hit_number=1):
    ui, ti = act(state, user), act(state, target)
    a_stats, d_stats = state.stats[user, ui], state.stats[target, ti]
    ...
    return cb.CbCtx(
        ...
        atk_status=state.status[user, ui], dfn_status=state.status[target, ti],
        atk_hp=state.hp[user, ui], atk_maxhp=state.maxhp[user, ui],
        dfn_hp=state.hp[target, ti], dfn_maxhp=state.maxhp[target, ti],
        atk_item=state.item[user, ui], dfn_item=state.item[target, ti],
        atk_weight=data["species_weight"][state.species[user, ui]],
        dfn_weight=data["species_weight"][state.species[target, ti]],
        ...
        terastallized=state.terastallized[user, ui],
        tera_type=state.tera_type[user, ui],
        fury_multiplier=jnp.int32(1),
        level=state.level[user, ui],
...
def before_move(data, state, user, move_id, key):
    ...
    asleep = state.status[user, ui] == C.SLP
    turns = jnp.maximum(state.status_turns[user, ui].astype(jnp.int32) - 1, 0)
    wakes = asleep & (turns <= 0)
    state = state._replace(
        status=state.status.at[user, ui].set(
            jnp.where(wakes, jnp.int8(C.STATUS_NONE), state.status[user, ui])),
        status_turns=state.status_turns.at[user, ui].set(
            jnp.where(asleep, turns.astype(jnp.int8), state.status_turns[user, ui])))
    ...
    frozen = state.status[user, ui] == C.FRZ
    thaws = jax.random.uniform(k_frz) < 0.2
    unfreeze = frozen & thaws
    state = state._replace(
        status=state.status.at[user, ui].set(
            jnp.where(unfreeze, jnp.int8(C.STATUS_NONE), state.status[user, ui])))
    can = can & jnp.logical_not(frozen & jnp.logical_not(thaws))

    # Full paralysis: 25%.
    paralysed = state.status[user, ui] == C.PAR
...
def powder_immune(data, state, target, move_id):
    ...
    return has_flag(data["move_flags"][move_id], "powder") & (
        jnp.any(types == C.GRASS) | (state.ability[target, ti] == A.OVERCOAT) |
        (state.item[target, ti] == I.SAFETYGOGGLES))
...
def _eff_substitute(data, state, user, target, key):
    ...
    enough = state.hp[user, ui].astype(jnp.int32) > cost
...
def _eff_rest(data, state, user, target, key):
    ui = act(state, user)
    full = state.hp[user, ui] >= state.maxhp[user, ui]
    ok = jnp.logical_not(full)
    state, _ = heal_pokemon(state, user, ui,
                            jnp.where(ok, state.maxhp[user, ui].astype(jnp.int32), 0))
    return state._replace(
        status=state.status.at[user, ui].set(
            jnp.where(ok, jnp.int8(C.SLP), state.status[user, ui])),
        status_turns=state.status_turns.at[user, ui].set(
            jnp.where(ok, jnp.int8(3), state.status_turns[user, ui])))
...
def _eff_painsplit(data, state, user, target, key):
    ui, ti = act(state, user), act(state, target)
    total = state.hp[user, ui].astype(jnp.int32) + state.hp[target, ti].astype(jnp.int32)
    half = total // 2
    new_u = jnp.minimum(half, state.maxhp[user, ui].astype(jnp.int32))
    new_t = jnp.minimum(half, state.maxhp[target, ti].astype(jnp.int32))
    return state._replace(hp=state.hp.at[user, ui].set(new_u.astype(jnp.int16))
                          .at[target, ti].set(new_t.astype(jnp.int16)))
...
def _eff_trick(data, state, user, target, key):
    ...
    ui, ti = act(state, user), act(state, target)
    a, b = state.item[user, ui], state.item[target, ti]
    ok = (a != 0) | (b != 0)
    return state._replace(
        item=state.item.at[user, ui].set(jnp.where(ok, b, a))
        .at[target, ti].set(jnp.where(ok, a, b)))


def _eff_knockoff(data, state, user, target, key):
    ti = act(state, target)
    return state._replace(item=state.item.at[target, ti].set(jnp.int16(0)))


def _eff_bellydrum(data, state, user, target, key):
    ui = act(state, user)
    cost = fraction_of_max(state, user, ui, 1, 2)
    ok = state.hp[user, ui].astype(jnp.int32) > cost
    state, _ = damage_pokemon(state, user, ui, jnp.where(ok, cost, 0))
...
def _eff_psychoshift(data, state, user, target, key):
    ui = act(state, user)
    status = state.status[user, ui]
    state, applied = set_status(data, state, target, status, key)
    return jax.lax.cond(applied, lambda s: cure_status(s, user, ui), lambda s: s, state)
...
EFFECT_FNS = {
    ...
    "smackdown": _eff_smackdown,
}

UNIMPLEMENTED_EFFECTS = tuple(h for h in E.EFFECT_HANDLERS if h not in EFFECT_FNS)

# vmap lowers `lax.switch` to "evaluate every branch and select", and each branch
# here returns an entire BattleState. Declaring 58 handlers but implementing 31
# would therefore cost nearly twice what it needs to, so the switch is built over
# the implemented handlers only and unimplemented ids are folded onto the no-op.
_IMPLEMENTED_EFFECTS = [h for h in E.EFFECT_HANDLERS if h in EFFECT_FNS]
_EFFECT_BRANCHES = tuple(EFFECT_FNS[h] for h in _IMPLEMENTED_EFFECTS)
_COMPACT_EFFECT_ID = jnp.asarray(
    [_IMPLEMENTED_EFFECTS.index(h) if h in EFFECT_FNS else 0
     for h in E.EFFECT_HANDLERS], jnp.int32)


def run_effect(effect_id, data, state, user, target, key):
    compact = _COMPACT_EFFECT_ID[jnp.clip(effect_id.astype(jnp.int32), 0,
                                          len(E.EFFECT_HANDLERS) - 1)]
    return jax.lax.switch(compact, _EFFECT_BRANCHES, data, state, user, target, key)
...
def _apply_secondaries(data, state, user, target, move_id, key):
    """Roll each of the move's up-to-two secondary effects."""
    ti = act(state, target)
    shielded = state.ability[target, ti] == A.SHIELDDUST
    covert = state.item[target, ti] == I.COVERTCLOAK
    serene = state.ability[user, act(state, user)] == A.SERENEGRACE
    ...
    for k in range(2):
        ...
        fires = (chance > 0) & (jax.random.randint(k_roll, (), 0, 100) < chance) & \
                jnp.logical_not(blocked)

        status = data["move_sec_status"][move_id, k]
        state, _ = jax.lax.cond(
            fires & (status != C.STATUS_NONE),
            lambda s: set_status(data, s, target, status, k_status),
            lambda s: (s, jnp.bool_(False)), state)

        vol = data["move_sec_volatile"][move_id, k]
        state = state._replace(volatiles=state.volatiles.at[target, vol].set(
            jnp.where(fires & (vol > 0), jnp.int8(1), state.volatiles[target, vol])))
...
def execute_move(data, state, user, move_slot, moves_first, key):
    ...
    ti = act(state, target)
    target_alive = state.hp[target, ti] > 0
    ...
    connects = (can_act & target_alive & hits_acc & jnp.logical_not(blocked) &
                jnp.logical_not(powder) & jnp.logical_not(terrain_block) &
    ...
    state, _ = heal_pokemon(state, target, ti,
                            jnp.where(absorbs & (absorb_heal > 0),
                                      ...
                                      0))
    boost_vec = jnp.zeros(7, jnp.int32).at[jnp.maximum(absorb_stat, 0)].set(
        jnp.where((absorb_stat >= 0) & absorbs, absorb_amt.astype(jnp.int32), 0))
    ...
    is_crit = ((jax.random.randint(k_crit, (), 0, 24) % crit_denom) == 0) & \
              jnp.logical_not(crit_immune) | data["move_will_crit"][move_id]
    is_crit = is_crit & jnp.logical_not(crit_immune)

    def one_hit(i, carry):
        st, total = carry
        active = (i < n_hits) & connects & jnp.logical_not(is_status)
        roll = jax.random.randint(jax.random.fold_in(k_roll, i), (), 0, 16)
        mv_i = scale_power_for_hit(data, move_id, mv, i + 1)
        dmg = calc_damage(
            data, atk, dfn, mv_i, is_crit=is_crit, damage_roll=roll,
            weather=effective_weather(st), terrain=st.terrain,
            side_conditions=st.side_conditions[target], type_exp=type_exp,
            bp_cb_mod=bp_cb_mod, grounded_user=cb_ctx.grounded_user,
            grounded_target=cb_ctx.grounded_target,
            analytic_ok=jnp.logical_not(moves_first),
            fainted_count=st.fainted_count[user].astype(jnp.int32))
        dmg = jnp.where(is_fixed, fixed, dmg)
        dmg = jnp.where(ohko, st.maxhp[target, ti].astype(jnp.int32), dmg)
        dmg = jnp.where(active, dmg, 0)
        # Focus Sash / Sturdy survive a full-HP KO with 1 HP.
        endures = ((st.item[target, ti] == I.FOCUSSASH) |
                   (st.ability[target, ti] == A.STURDY)) & \
                  (st.hp[target, ti] == st.maxhp[target, ti]) & \
                  (dmg >= st.hp[target, ti].astype(jnp.int32))
        dmg = jnp.where(endures, st.hp[target, ti].astype(jnp.int32) - 1, dmg)
        st, dealt = _apply_hit_damage(st, user, target, dmg, bypass_sub)
        return st, total + dealt

    state, total_damage = jax.lax.fori_loop(0, 10, one_hit, (state, jnp.int32(0)))
    ...
    state = state._replace(volatiles=state.volatiles.at[target, vol].set(
        jnp.where(applied & (vol > 0), duration, state.volatiles[target, vol])))
    self_vol = data["move_self_volatile"][move_id]
    state = state._replace(volatiles=state.volatiles.at[user, self_vol].set(
        jnp.where(can_act & (self_vol > 0), duration, state.volatiles[user, self_vol])))
    ...
    state, _ = apply_boosts(state, target,
                            jnp.where(applied & jnp.logical_not(self_targeting), tgt_boosts, 0),
                            from_opponent=True)
    ...
    state = jax.lax.cond(landed,
                         lambda s: _apply_secondaries(data, s, user, target, move_id, k_sec),
                         lambda s: s, state)
    effect_id = data["move_effect_cb"][move_id]
    state = jax.lax.cond(
        can_act & (effect_id > 0),
        lambda s: run_effect(effect_id, data, s, user, target, k_eff),
        lambda s: s, state)
    ...
    # Choice items lock the user into the move it just used.
    choiced = data["item_is_choice"][state.item[user, ui]]
    state = state._replace(choice_slot=state.choice_slot.at[user].set(
        jnp.where(choiced & can_act, move_slot.astype(jnp.int8), state.choice_slot[user])))

    # U-turn / Volt Switch and whirlwind-style forced switches.
    self_switch = (data["move_self_switch"][move_id] > 0) & landed
    force_switch = data["move_force_switch"][move_id] & landed
    state = state._replace(force_switch=state.force_switch
                           .at[user].set(state.force_switch[user] | self_switch)
                           .at[target].set(state.force_switch[target] | force_switch))
    return state
...
def _set_side_condition(state, side, condition, apply_it):
    ...
    current = state.side_conditions[side, condition].astype(jnp.int32)
    caps = jnp.ones(C.NUM_SIDE_CONDITIONS, jnp.int32) * 5
    for idx, cap in C.SIDE_CONDITION_MAX.items():
        caps = caps.at[idx].set(cap)
    is_hazard = condition < C.SC_REFLECT
    new = jnp.where(is_hazard, jnp.minimum(current + 1, caps[condition]), 5)
    return state._replace(side_conditions=state.side_conditions.at[side, condition].set(
        jnp.where(apply_it, new.astype(jnp.int8), current.astype(jnp.int8))))
...
def _roll_hit_count(data, state, user, move_id, key):
    ...
    loaded = (state.item[user, ui] == I.LOADEDDICE) & (hi > lo)
